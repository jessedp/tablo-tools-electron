...
export default class DbStats extends Component<Props, State> {
  ...
  async refresh() {
    ...
    const watchedData = [
      { id: 'watched', label: 'watched', value: watched },
      { id: 'unwatched', label: 'unwatched', value: recTotal - watched }
    ];
    ...
    const stateData = [
      ...
      { id: 'in-progress', label: 'in progress', value: recording }
    ];
    ...
    const typeProgram = await RecDb.asyncCount({ path: { $regex: recType } });

    const typeData = [
      { id: 'episode', label: 'episode', value: typeEpisode },
      { id: 'movie', label: 'movie', value: typeMovie },
      { id: 'event', label: 'event', value: typeEvent },
      { id: 'manual', label: 'manual', value: typeProgram }
    ];
