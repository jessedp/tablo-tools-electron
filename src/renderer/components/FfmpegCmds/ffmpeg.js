...
function setVideoFlags(options) {
  ...
  if (options.codecOptions && ['libx264', 'libx265'].includes(options.vcodec)) {
    const arg = [
      `-${options.vcodec.replace('lib', '')}-params`,
...
function setAudioFlags(options) {
  ...
  if (options.quality && options.quality !== 'auto') {
    const arg = [
      '-b:a',
      options.quality === 'custom' ? options.audioBitrate : options.quality,
    ];
    flags.push(...arg);
  }
  return flags;
...
function buildFlags(opt, forExport = false) {
  const options = opt || {};
  ...
  // Set output.
  if (!forExport) extra.push(output);
